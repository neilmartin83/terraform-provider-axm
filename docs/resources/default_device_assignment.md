...
- `watch` (String) MDM server ID to assign as the default for Apple Watch devices. Set to "" to unassign from the server currently tracked in Terraform state. Omit (null) to leave this family unmanaged. Note: if a family was assigned to a server outside Terraform and is not in state, setting "" will not clear it — adopt the existing assignment first by setting this attribute to the current server ID, then change it as needed.
